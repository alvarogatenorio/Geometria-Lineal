...
\begin{proof}
	...
	Buscamos probar que el conjunto de vectores de $E$ que son anulados por todas las formas lineales de $W$ es un subespacio vectorial de $E$. A este subespacio le denominaremos \ti{antianulador} o \ti{anulador dual}.
	
	Notamos que $W$ tendrá una cierta base compuesta de $r$ formas lineales. Esto tiene importancia, ya que, cada vector que sea anulado por todas las formas lineales de la base, también lo será, por linealidad, por todas las aplicaciones de $W$.
	...
	Así, pues el conjunto de los vectores tales que son anulados por las formas lineales de $W$ (antianulador) es el conjunto de vectores que cumplen las ecuaciones:
	\[\left\{\begin{array}{ccc}
	a_1^1x_1+\dots+a_n^1x_n&=&0\\
	\ddots& \vdots & \vdots\\
	a_1^rx_1+\dots+a_n^rx_n&=&0
	\end{array}\right.\]
	Estas ecuaciones pueden interpretarse por las ecuaciones cartesianas de cierto subesacio $U$ (único) de dimensión $n-r$ cuyo anulador es precisamente $W$.
...
\begin{equation}
	\dim(U^*)+\dim(U^{*\perp})=\dim(E^*)
\end{equation}
Dicho de otra forma, un subespacio $U$ de dimensión $r$ ``dualiza'' en un subespacio $U^\perp$ de dimensión $n-r$. Esto nos recuerda, de alguna manera, a la idea de complemento ortogonal (a pesar de no tener ningún producto escalar que nos respalde). 

Recapitulando, hemos conseguido construir un puente entre las variedades de un espacio vectorial y las de su dual ``vía anuladores''.

Trataremos de justificar a continuación que la operación de ``paso al dual'' es una \ti{involución}, es decir, que si consideramos $E^*$ como espacio vectorial original, y tomamos una variedad suya $W^*$, su anulador $W^{*\perp}\subset E^{**}$ coincide con su antianulador $W\subset E$. Es decir:
\begin{equation}
	\{\alpha\in E^{**}\tq \alpha(u)=0\forall u\in E*\}\stackrel{\mathrm{?}}{=}\{\alpha\in E^*\tq \alpha(u)=0\forall u\in E\}
\end{equation}

Esto resulta confuso, ya que estamos tratando de identificar un subespacio del \ti{bidual} $E^{**}$ con otro subespacio de $E$. Sin embargo, al ser espacios de la misma dimensión, podemos identificarlos (son isomorfos).
