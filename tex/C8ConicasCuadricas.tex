...
\end{defi}
Es claro que el haz de cónicas engendrado por $\mc{C}_1$ y $\mc{C}_2$ viene dado por las siguientes ecuaciones paramétricas homogéneas
\[\mc{C}_1\mc{C}_2\equiv\haz_{\mc{C}_1,\mc{C}_2}:\alpha\mc{C}_1+\beta\mc{C}_2=0\]
Usualmente usaremos la deshomogeneización habitual dividiendo por $\alpha$ y considerando un parámetro no homogéneo.
\[\begin{array}{cc}\haz_{\mc{C}_1,\mc{C}_2}:\mc{C}_1+\frac{\beta}{\alpha}\mc{C}_2\stackrel{\mathrm{not.}}{=}\mc{C}_1+\lambda\mc{C}_2\qquad&\qquad\lambda\in\overline{\K}=\K\cup\{\infty\}\end{array}\]
...
\begin{proof}
	...
	Distinguimos casos entre si hay agun punto que pueda no estar en el triangulo y si no hay más remedio que esté
	
	Es una distinción de casos bastante trivial
...
\end{equation}
Si el punto $P$ fuese interior a la cónica, los puntos de corte de las rectas del haz con base $P$ y la cónica $\mc{C}$ serían puntos imaginarios conjugados.
\begin{obs}
...
\begin{exa}[Aplicación del Teorema de Steiner] REVISAR CUENTAS
...
Es lógico pensar que la razón doble de cuatro rectas de la cónica dual, que son rectas tangentes a $\mc{C}$ en un punto, sea igual que la razón doble de los puntos de tangencia. Recordemos que estos puntos de tangencia eran los polos de las rectas tangentes. Por tanto, nos gustaría que, si $P_i=C^*l_i$, entonces $\{l_1,l_2;l_3,l_4\}=\{P_1,P_2;P_3,P_4\}$, donde $C^*$ es la adjunta de la matriz de la cónica. Para llegar a este resultado necesitamos un lema previo.
\begin{lem}
