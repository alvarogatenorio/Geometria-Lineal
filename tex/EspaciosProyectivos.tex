...
Podemos aproximarnos de dos formas distintas a la noción de ``\tbi[espacio!proyectivo]{espacio proyectivo}'', como veremos, ambas aproximaciones son equivaletes, de modo que las confundiremos cuando nos convenga. Cabe destacar que la segunda de estas aproximaciones tiene especial interés desde el punto de vista de la \tbi{topología}, por estar basada en conjuntos cociente, pero esos asuntos se salen del alcance de estas notas.
\subsection{Primera aproximación}
Antes de comenzar, cabe mencionar que para referirnos a los espacios vectoriales, a veces usaremos el termino ``\tbi[espacio!lineal]{espacio lineal}''.
...
\end{defi}
Esta definición, presentada así, es como un diamante en bruto que debemos pulir un poco si lo que pretendemos es extraer algo útil de ella, motivo por el cual presentamos la siguiente definición.
\begin{defi}[Rayo]
	\label{epro_def_rayo}
...
\begin{proof}\begin{itemize}
		\item[\bsupset] Como $A\subset\engen{A}$ entonces $\pi^{-1}(A)\subset\widehat{\engen{A}}\ra\lengen{\pi^{-1}(A)}\subset\widehat{\engen{A}}$
